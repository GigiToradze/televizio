...
function Loading() {
  const { t } = useLang();
  return <div className="gate"><p className="eyebrow">{t('იტვირთება', 'Loading')}</p></div>;
}
    ...
    return (
      <div className="gate">
        <div className="gate__form">
          <p className="eyebrow">{t('წვდომა არ არის', 'No access')}</p>
          <p>{t(
            'ეს ანგარიში შესულია, მაგრამ ადმინი არ არის.',
            'This account is signed in but is not an admin.',
          )}</p>
          <p className="note">{t(
            'სთხოვე მფლობელს, დაგამატოს პარამეტრებიდან.',
            'Ask an owner to add you from Settings.',
          )}</p>
